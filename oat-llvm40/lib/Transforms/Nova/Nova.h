...
namespace llvm{
    ...
    // AliasObject members
    typedef struct AliasObject *AliasObjectRef;//##ref都是结构体的指针的类型别名
    typedef SetVector<AliasObjectRef> AliasObjectSet;
    typedef std::map<int, AliasObjectSet *> AliasMap;//是一个图，key是int类型的，value是aliasobjectref组成的一个vector
    typedef AliasMap *AliasMapRef;
    typedef std::map<int, InstSet *> LocalTaintMap;//是一个图，key是int类型的，value是Instruction组成的一个vector
...
}; // struct AliasObjectTuple ，用于记录一个别名的位置
