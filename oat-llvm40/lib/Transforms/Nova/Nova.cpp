...
void Nova::ReverseSCC(std::vector<SCCRef> &sccVector, Function *f) {
    for (scc_iterator<Function *> I = scc_begin(f), //遍历函数中的基本块
                                  IE = scc_end(f);
                                  I != IE; ++I) {
        ...
        reverse(tmpSCC->begin(), tmpSCC->end());//反转tmpscc
        sccVector.push_back(tmpSCC);
    }
    
    reverse(sccVector.begin(), sccVector.end());//反转sccVector
...
void Nova::UpdatePtoLoad(GlobalStateRef gs, Instruction &I){
    ...
    op = li->getPointerOperand();//获取load指令的操作数
...
//然后遍历aliasobjectset集合，将里面的aliasobject加入到senvarset中
        for (TupleSet::iterator tsit = ts->begin(), tsie = ts->end();
            ...
            //处理结构体类型变量
            if ((*tsit)->ao->type != NULL && (*tsit)->ao->type->isStructTy()) {//处理结构体类型的敏感变量
                // print struct field aliasMap
                for (AliasMap::iterator ait = aliasMap->begin(), aie = aliasMap->end();
                                                ait != aie; ++ait) {
    ...
    for (PointsToMap::iterator it = gs->pMap->begin(), ie = gs->pMap->end();//it是一个PointToMapRef，其组成为map<Value*, TupleSet *>
                                                         it != ie; ++it) { 
        ...
        for (TupleSet::iterator tsit = it->second->begin(), tsie = it->second->end();//tsit是一个AliasObjectTuple
                                                    tsit != tsie; ++tsit) {
            if ((*tsit) == NULL || (*tsit)->ao == NULL) {
                errs() << "(*tsit)->ao == NULL!\n";
                continue;
        ...
        if (Instruction *Inst = dyn_cast<Instruction>(UoV)) {// 如果用户是指令
            ...
            if (isa<StoreInst>(Inst)){
                ...
                if (op == var) {//如果 Store 操作的地址操作数等于变量 var
                    // define event :insert call to void record_defevt(uint64 addr, uint64 val)如果存储的地址是变量 var 本身，插入记录定义事件的代码
...
// 获取或插入名为 "__record_defevt" 的函数
    ...
    Function *RecordDefEvtFunc = cast<Function>(RecordDefEvt);
    castAddr = CastInst::Create(Instruction::PtrToInt, addr, I64Ty, "recptrtoint", inst);// 将地址值转换为 64 位整数,inst表示在哪条指令之前插入新创建的指令

    if (val->getType()->isPointerTy()) {
        castVal = CastInst::Create(Instruction::PtrToInt, val, I64Ty, "recptrtoint", inst);// 如果值的类型是指针，将其转换为 64 位整数
    } else if (val->getType()->isIntegerTy(64)) {// 如果值的类型是 64 位整数，直接使用该值
        castVal = val;
    } else if (val->getType()->isIntegerTy()) {
        castVal = CastInst::Create(Instruction::ZExt, val, I64Ty, "reczexttoi64", inst);// 如果值的类型是整数，将其零扩展为 64 位整数
    } else
	return;
...
void Nova::InstrumentLoadInst(Instruction *inst, Value *addr, Value *val) {
    ...
    castAddr = CastInst::Create(Instruction::PtrToInt, addr, I64Ty, "chkptrtoint", inst);// 将地址值转换为 64 位整数

    if (val->getType()->isPointerTy()) {
        //errs() << "val cast from pointer to int64\n";
        //errs() << "val:"<< *val;
        castVal = CastInst::Create(Instruction::PtrToInt, val, I64Ty, "chkptrtoint", inst);// 如果值的类型是指针，将其转换为 64 位整数
    } else if (val->getType()->isIntegerTy(64)) {
        castVal = val; 
    } else if (val->getType()->isIntegerTy()) {
        //errs() << "val cast from other to int64\n";
        //errs() << "val:"<< *val;
        castVal = CastInst::Create(Instruction::ZExt, val, I64Ty, "chkzexttoi64", inst);// 如果值的类型是整数，将其零扩展为 64 位整数
    } else
        return;
        ...
        if (Instruction *Inst = dyn_cast<Instruction>(UoV)) {
            if (isa<LoadInst>(Inst)){
                ...
                if (op == var) {
                    ...
                    fakeInst = CastInst::Create(Instruction::PtrToInt, op, I64Ty, "fptrtoint"); // 创建一个伪指令，将地址操作数转换为 64 位整数
                    pb = Inst->getParent();// 获取当前指令所在的基本块
                    assert(pb != nullptr);
