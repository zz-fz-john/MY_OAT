...
static cl::opt<bool> CollectCondBranchHintsInfo("collect-cond-branch-hints", cl::Hidden,//负责将选项加入命令行，然后用户可以通过命令行来配置
                                  cl::init(true));
...
struct  CollectCondBranchHints : public FunctionPass {
  ...
  CollectCondBranchHints() : FunctionPass(ID) {
    ...
    std::ifstream infile(InstrumentFunctionNameListFile);//从文件中读取配置
    std::string word;
    while(infile>>word) {
        list->push_back(word);
    }
  ...
  bool runOnFunction(Function &F) override {
    ...
    errs() << __func__ << " : "<< name << "\n";
    if (std::find(std::begin(*list), std::end(*list), name) == std::end(*list)) {//没有找到名字为name的函数，则为真，则执行循环
